...
- [terraform-provider-uptimepage](https://github.com/uptimepage/terraform-provider-uptimepage) - Provider for [Uptimepage](https://uptimepage.dev/) uptime monitoring: monitors, status pages and notification channels.
...
- [Datadef](https://datadef.io/repo-to-diagram) - Generates architecture diagrams and docs from a Terraform repo: parses `.tf` files without running `terraform init` or reading state, draws modules as zones with per-environment counts, re-syncs daily. :heavy_dollar_sign:
...
- [tfmodcache](https://github.com/Rezarys/tfmodcache) - Local, shared module cache for Terraform and OpenTofu; `terraform init` stops re-downloading modules it already has. I am the author.
